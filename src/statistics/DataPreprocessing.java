...
public class DataPreprocessing {
	...
	for (PaasProfile profile : profiles) {
		...
		revisionData.addRevision(profile.getName(), ChronoUnit.DAYS
			.between(LocalDate.parse(profile.getRevision()
				.substring(0, 10)), LocalDate.now()));
	...
	profiles.forEach(profile -> {
	    if (!(profile.getPlatform() == null || profile.getPlatform()
		    .equals("null"))) {
	...
	profiles.forEach(profile -> {
	    ...
	    profile.getPricings()
		    .forEach(
			    profilePricing -> {
				if (profilePricing.getModel().equals(
					PricingModel.free)) {
				    pricingData.incrementFreeModels();
				} else if (profilePricing.getModel().equals(
					PricingModel.fixed)) {
				...
				if (profilePricing.getPeriod().equals(
					PricingPeriod.daily)) {
				    pricingData.incrementDailyPeriod();
				} else if (profilePricing.getPeriod().equals(
					PricingPeriod.monthly)) {
				    pricingData.incrementMonthlyPeriod();
				} else if (profilePricing.getPeriod().equals(
					PricingPeriod.anually)) {
	...
	for (PaasProfile profile : profiles) {
	    runtimesData.addProfileRuntimeAmount(profile.getName(), profile
		    .getRuntimes().size());
	...
	for (PaasProfile profile : profiles) {
		...
		middlewares
			.replace(
				"midAmount|" + numberOfMiddlewares,
				middlewares.get("midAmount|"
		...
		profileMiddleware.getVersions()
			.forEach(
				version -> {
				    String key = profileMiddleware.getName()
					    + "|" + version;
				    if (middlewareData.putIfAbsent(key,
					    (long) 1) != null) {
					middlewareData.replace(key,
						middlewareData.get(key),
	...
	profiles.forEach(profile -> {
	    profile.getFrameworks().forEach(
		    framework -> {
			framework.getVersions().forEach(
				version -> {
				    String key = framework.getName() + "|"
					    + framework.getRuntime() + "|"
					    + version;
				    if (frameworksData.putIfAbsent(key,
					    (long) 1) != null) {
					frameworksData.replace(key,
						frameworksData.get(key),
	...
	profiles.forEach(profile -> {
		...
		extensibleData.replace("true", extensibleData.get("true"),
			extensibleData.get("true") + 1);
	    }
	    if (profile.getExtensible().equals("false")) {
		extensibleData.replace("false", extensibleData.get("false"),
			extensibleData.get("false") + 1);
	    }
	    if (profile.getExtensible().equals("unknown")) {
		extensibleData.replace("unknown",
			extensibleData.get("unknown"),
			extensibleData.get("unknown") + 1);
	    }
	});
    }
	...
	profiles.forEach(profile -> {
	    profile.getInfrastructures()
		    .forEach(
			    infrastructure -> {
				String key = infrastructure.getContinent()
					+ "|" + infrastructure.getCountry()
					+ "|" + infrastructure.getRegion()
					+ "|" + infrastructure.getProvider();
				if (infrastructuresData.putIfAbsent(key,
					(long) 1) != null) {
				    infrastructuresData.replace(key,
					    infrastructuresData.get(key),
					    infrastructuresData.get(key) + 1);
				}
			    });
	});
    }
